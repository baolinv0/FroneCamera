...
def test_pipeline_quick_mode_creates_final_report_without_blocking_on_reviews(
    tmp_path: Path,
) -> None:
    folders = []
...
def test_pipeline_classifies_external_professional_corroboration(tmp_path: Path) -> None:
    ...
    class SearchStub(SearchProvider):
        def search(self, query: str, limit: int = 5) -> list[ExternalEvidence]:
            return [
                ExternalEvidence(
                    title="Device 1 selfie camera review",
                    url=f"https://example.test/{abs(hash(query))}",
                    source_domain="example.test",
                    snippet="The front camera keeps the face bright in matched scenes.",
                )
            ]

    class CorroborationStub(CorroborationAdapter):
        def classify(
            self, claim: str, device_name: str, evidence: ExternalEvidence
        ) -> CorroborationDecision:
            return CorroborationDecision(
                verdict="supports",
                confidence=0.9,
                reason="Direct front-camera evidence matches the output tendency.",
    ...
    with database.session_factory() as session:
        ...
        repo.confirm_pairing(project.id, pairing["version"])
        EvaluationPipeline(
            session, tmp_path / "workspace", search=SearchStub(), corroborator=CorroborationStub()
        ).run(project.id)
    ...
    class RecordingAdapter(VisionModelAdapter):
        ...
        def analyze(self, scene_id, image_paths, context):  # type: ignore[no-untyped-def]
            ...
            return ModelEvaluationResult(
                scene_id=scene_id, role=self.role, observations=[], confidence=0.8
            )
...
def test_pipeline_creates_review_item_when_reviewer_challenges_primary_observation(
    tmp_path: Path,
) -> None:
            ...
            if context["pass"] == "metric_validation":
                ...
                observations = [
                    ModelObservation(
                        ...
                        dimension="highlight_retention",
                        statement="This output has more retained highlight structure.",
                        evidence_refs=[f"asset:{scene_id}:{device_code}"],
                        certainty=0.8,
                    )
                ]
            return ModelEvaluationResult(
                scene_id=scene_id, role="primary", observations=observations, confidence=0.8
            )

    class ReviewerAdapter(VisionModelAdapter):
        def analyze(self, scene_id, image_paths, context):  # type: ignore[no-untyped-def]
            return ModelEvaluationResult(
                scene_id=scene_id, role="reviewer", observations=[], confidence=0.8
            )
    ...
    with database.session_factory() as session:
        ...
        repo.confirm_pairing(project.id, pairing["version"])
        EvaluationPipeline(
            session, tmp_path / "workspace", primary=PrimaryAdapter(), reviewer=ReviewerAdapter()
        ).run(project.id)
