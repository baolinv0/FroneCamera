...
def test_pairwise_retries_are_append_only_and_require_latest_parent() -> None:
    ...
    try:
        ...
        assert repository.get_latest_pairwise_attempt("batch-attempts", "comparison-001") == second
        assert repository.list_pairwise_attempts("batch-attempts", "comparison-001") == [
            first,
            second,
        ]
