...
def payload(
    luma: float, face_bbox: list[int], size: tuple[int, int] = (100, 100)
) -> dict[str, object]:
    return {
        "whole": {
            "luma_mean": luma,
            "luma_p90": min(1.0, luma + 0.2),
            "luma_p10": max(0.0, luma - 0.2),
            "highlight_clip_ratio": 0.0,
            "apparent_detail": 100.0,
        },
        "face_bbox": face_bbox,
...
def test_rendered_brightness_difference_is_observation_not_capture_confounder() -> None:
    audit = audit_scene(
        {"a": payload(0.15, [25, 20, 40, 50]), "b": payload(0.75, [25, 20, 40, 50])}
    )
