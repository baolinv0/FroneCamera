...
def test_confirm_pairing_creates_immutable_versioned_snapshot(tmp_path: Path) -> None:
    ...
    with database.session_factory() as session:
        ...
        devices = [
            ...
            for name, folder in zip(("A", "B"), folders, strict=True)
        ]
        ...
        changed = repo.update_pairing_cell(
            project.id,
            confirmed["groups"][0]["id"],
            devices[0].id,
            available[1]["image_id"],
            confirmed["version"],
        )
        ...
        assert (
            snapshots[1]["payload"]["groups"][0]["cells"][devices[0].id]["image_id"]
            == original_image
        )
