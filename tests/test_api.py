...
def test_project_device_scan_pairing_flow(tmp_path: Path) -> None:
    ...
    settings = Settings(
        database_url=f"sqlite:///{tmp_path / 'test.db'}", workspace=tmp_path / "workspace"
    )
    ...
    for name, folder in [("A", device_a), ("B", device_b)]:
        response = client.post(
            f"/api/projects/{project_id}/devices", json={"name": name, "folder_path": str(folder)}
        )
    ...
    confirmed = client.post(
        f"/api/projects/{project_id}/pairing/confirm", json={"expected_version": pairing["version"]}
    )
...
def test_diagnostic_asset_is_served_only_from_workspace(tmp_path: Path) -> None:
    settings = Settings(
        database_url=f"sqlite:///{tmp_path / 'asset.db'}", workspace=tmp_path / "workspace"
    )
    settings.prepare()
    client = TestClient(create_app(settings))
    ...
    with database.session_factory() as session:
        row = Repository(session).save_analysis(
            project["id"], "image_metrics", {"diagnostic_path": str(diagnostic)}
        )
        analysis_id = row.id
...
def test_report_pdf_endpoint_uses_registered_report(tmp_path: Path, monkeypatch) -> None:
    settings = Settings(
        database_url=f"sqlite:///{tmp_path / 'pdf.db'}", workspace=tmp_path / "workspace"
    )
    settings.prepare()
    client = TestClient(create_app(settings))
...
def test_report_html_endpoint_returns_requested_version(tmp_path: Path) -> None:
    settings = Settings(
        database_url=f"sqlite:///{tmp_path / 'html.db'}", workspace=tmp_path / "workspace"
    )
    settings.prepare()
    client = TestClient(create_app(settings))
...
def test_finalize_regenerates_bundle_and_applies_rejected_attribution_review(
    ...
    settings = Settings(
        database_url=f"sqlite:///{tmp_path / 'reviewed-report.db'}",
    ...
    payload = ReportPayload(
        ...
        devices=["A", "B"],
        findings=[{"statement": "Observable finding", "grade": "B", "evidence": ["G001"]}],
        attributions=[
            {
                "statement": "Mechanism hypothesis",
                "device_id": "device-a",
                "primary_layer": "rendering",
            }
        ],
    )
    ...
    with database.session_factory() as session:
        ...
        review = repo.create_review_item(
            project["id"],
            "mechanism_attribution_review",
            {"statement": "Mechanism hypothesis", "device_id": "device-a"},
        )
        review_id = review.id
    resolved = client.patch(
        f"/api/review-items/{review_id}", json={"status": "rejected", "note": "unsupported"}
    )
...
def test_run_full_evaluation_passes_selected_mode_to_pipeline(tmp_path: Path, monkeypatch) -> None:
    settings = Settings(
        database_url=f"sqlite:///{tmp_path / 'workflow.db'}", workspace=tmp_path / "workspace"
    )
    client = TestClient(create_app(settings))
    called: dict[str, str] = {}
