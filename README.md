...
```text
N device folders
→ tiered automatic scene pairing
→ low-confidence pairing review and confirmation
...
Filenames may differ. The tiered matcher combines raw-file placeholders, explicit numeric ordinals, capture order, and content-sequence alignment. High-confidence groups are accepted automatically; only groups marked `REVIEW` need focused inspection before confirmation. A missing capture remains an explicit empty cell, so later groups do not silently shift.
...
## Automatic scene pairing

`Scan & propose pairing` now uses a tiered matcher instead of blindly zipping natural-sort positions. It first preserves unsupported DNG files as missing sequence slots, then uses explicit numeric ordinals such as `1.jpg`, `2.jpg`, and `4.jpg`, then equal-count capture order, and finally tone-normalized image-content sequence alignment for ambiguous folders. Every group carries `matching_confidence`, `review_required`, and diagnostic notes. The review console shows green `AUTO` badges and limits manual attention to red `REVIEW` groups.

## OpenAI/ChatGPT vision branch

The existing local open-model branch remains the default. To analyze the same anonymous matched groups with the official OpenAI Responses API, configure:

```env
PORTRAIT_EVAL_VLM_PROVIDER=openai
PORTRAIT_EVAL_OPENAI_API_KEY=replace-with-your-api-key
PORTRAIT_EVAL_OPENAI_BASE_URL=https://api.openai.com
PORTRAIT_EVAL_OPENAI_PRIMARY_MODEL=gpt-5.6-sol
PORTRAIT_EVAL_OPENAI_REVIEWER_MODEL=gpt-5.6-sol
```

The OpenAI branch preserves per-scene device anonymization, the blind visual pass, metric-validation pass, independent reviewer pass, and challenge pass. Images are resized to the configured `PORTRAIT_EVAL_VLM_MAX_IMAGE_EDGE` and sent as JPEG data URLs. The key resolver checks the explicit setting, `OPENAI_API_KEY`, and finally the `OPENAI_API_KEY` field in `~/.codex/auth.json`; OAuth access and refresh tokens are never used. Selecting the OpenAI branch without a compatible API key fails explicitly instead of silently using the heuristic adapter.

For a low-cost connectivity and prompt smoke test before a complete evaluation, run one matched position:

```bash
python scripts/test_openai_scene.py /data/front-test --scene-index 1 --dry-run
python scripts/test_openai_scene.py /data/front-test --scene-index 1
```

The second command sends the automatically matched images to the OpenAI API and requires `PORTRAIT_EVAL_OPENAI_API_KEY`, `OPENAI_API_KEY`, or a compatible field in `~/.codex/auth.json`. A full six-device, 24-group pipeline invokes four multimodal passes per comparable group, so validate pairing and expected API usage before running it.
