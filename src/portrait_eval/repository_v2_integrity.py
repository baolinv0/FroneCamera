...
class V2Repository(BaseV2Repository):
    ...
    def _require_batch_image(
        ...
        *,
        ...
        self._require_batch_device(
            batch_id, image.device_id, message="image does not belong to batch"
        )
        return image
    ...
    def save_evaluation_batch(self, batch: EvaluationBatchV2) -> EvaluationBatchV2:
        ...
        if {device.id for device in devices} != set(batch.device_ids) or any(
            device.project_id != batch.project_id for device in devices
        ):
    ...
    def save_rough_ranking(self, batch_id: str, ranking: RoughRankingV2) -> RoughRankingV2:
        ...
        if (
            len(ordered_devices) != len(set(ordered_devices))
            or set(ordered_devices) != batch_devices
        ):
    ...
    def _validate_score_judge_lineage(
        ...
        if (
            ...
            or score.device_id not in {comparison.device_a_id, comparison.device_b_id}
        ):
